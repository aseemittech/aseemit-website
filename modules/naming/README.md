...
This terraform module is designed to generate consistent naming convention and tags for resources. You can use this module to implement a strict naming convention.
...
module "naming" {
  ...
  project_prefix = "adex"
  app_name       = "debugging-data"
  app_name_short = "debug"
  environment    = "dev"
}

resource "aws_s3_bucket" "debugging_data" {
  bucket = module.naming.resources.s3.name
  tags   = module.naming.resources.s3.tags
}
```
## Tagging
Tags that should be implemented on each resource created for the project and it should be defined at the aws provider level. Following default tags that are used by this module which will be merged to other tags as `tags = merge(local.shared_tags, var.tags)` :
```
...
| <a name="input_namespace"></a> [namespace](#input\_namespace) | ID element. Usually an abbreviation of your organization name, e.g. 'eg' or 'ai', to help ensure generated IDs are globally unique | `string` | `null` | no |
...
| <a name="output_delimiter"></a> [delimiter](#output\_delimiter) | Delimiter between `namespace`, `environmet`, `name` and `attributes` |
