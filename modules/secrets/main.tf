...
data "aws_iam_policy_document" "this" {
  ...
  #checkov:skip=CKV_AWS_356: Here we must specify all resources
  #checkov:skip=CKV_AWS_111: As we allow all resources we cant be specific constraints
  statement {
    effect    = "Allow"
    actions   = ["secretsmanager:CreateSecret", "secretsmanager:UpdateSecret", "secretsmanager:PutSecretValue", "secretsmanager:DeleteSecret"]
    resources = ["*"]
